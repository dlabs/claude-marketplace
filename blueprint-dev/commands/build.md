...
```
/blueprint-dev:build "user auth"
/blueprint-dev:build   # (implements the most recent plan)
```

## Prerequisites

- Recommended: run `/blueprint-dev:plan` first
- Recommended: run `/blueprint-dev:architect` for significant features
- Optional: run `/blueprint-dev:design` for UI features with A/B variants
...
5. **Next steps**: Run `/blueprint-dev:review` for multi-agent review, or `/blueprint-dev:ship` to merge
...
- Run `/blueprint-dev:review` before `/blueprint-dev:ship` for comprehensive review
